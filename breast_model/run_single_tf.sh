...
python src/cropping/crop_single.py \
    --mammogram-path $1 \
    --view $2 \
...
python src/optimal_centers/get_optimal_center_single.py \
    --cropped-mammogram-path ${SAMPLE_SINGLE_OUTPUT_PATH}/cropped.png \
    --metadata-path ${SAMPLE_SINGLE_OUTPUT_PATH}/cropped_metadata.pkl

echo 'Stage 3: Generate Heatmaps'
python src/heatmaps/run_producer_single_tf.py \
...
python src/modeling/run_model_single_tf.py \
    --view $2 \
